...
| 1.1 | `dd3b8ee` | 2026-03-02 | 我的旅程体系 | 新增旅程创建/管理/单旅程展示，支持旅程内地点增删 |
| 1.2 | `da3cdcd` | 2026-03-02 | 旅程视图稳定性与历史删除增强 | 修复单点旅程无法退出、单旅程删除边界，新增单条历史记录删除 |
| 1.3 | `054a696` | 2026-03-05 | 旅程分享图导出 | 新增旅程分享按钮，导出地图+地点清单 PNG 图片 |
...
1. 历史记录新增“删除该记录”按钮，可删除任意单条地点。
2. 删除单条地点后自动刷新旅程映射与旅程统计，避免视图残留。

修复问题：

1. 修复“仅有 1 个总地点且进入旅程视图后无法退出”的状态问题。
2. 修复“仅有 1 个旅程时删除失败”的边界问题。
3. 修复仅支持“撤回最后一条”而无法删除中间历史点的问题。
...
1. 在旅程视图横幅新增“分享旅程”按钮。
2. 点击后生成并下载固定尺寸 `2048x1152` 的 PNG 图片。
3. 分享图左侧为旅程地图：
   1. 包含底图、路径连线、全部地点。
   2. 地图点位显示编号（`#1, #2, ...`）。
4. 分享图右侧为地点列表：
   1. 展示“编号 + 地点名 + 时间”。
   2. 编号顺序与地图点位一致（按 `visited_at` 升序）。
5. 长列表自动收缩字号，超出高度时显示截断提示。
6. 导出过程包含状态提示与按钮禁用控制，避免重复触发。

解决问题：

1. 解决“旅程只能在页面内查看，无法一键分享”的问题。
2. 解决“分享时缺少路径与顺序语义”的问题。
3. 解决“手机端下载兼容性不稳定”的问题（增加 iOS 回退打开）。
...
10. 单条历史地点删除（不仅限最后一条）。
11. 旅程分享图导出（地图路径+编号点+地点清单 PNG 下载）。
12. 手机/桌面响应式布局。

---

## 6. 1.3 对应数据库要求（Supabase）
...
1. 无新增表结构与 RLS 策略变更。
2. 单条历史删除复用既有 `travel_points` 删除能力与外键级联逻辑。

### 6.4 版本 1.3 数据库变更

1. 无新增表结构、索引和 RLS 策略变更。
2. 分享图导出为纯前端能力，不新增云端存储或分享链接表。

---

## 7. 结论

从 `0.1` 到 `1.3`，项目已从“位置记录工具”升级为“可登录、可同步、可追踪、可编辑语义地点、可按旅程组织和复盘，并支持精细化历史删除与一键导出旅程分享图”的完整个人足迹系统。
